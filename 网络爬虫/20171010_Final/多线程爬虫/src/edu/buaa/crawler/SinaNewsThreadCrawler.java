...
public class SinaNewsThreadCrawler {
	...
	private static String SAVE_PATH = "/Users/kkb/Documents/sina_data/";

	public static void main(String[] args) {
		allUrlDepth.put("http://news.sina.com.cn", 1);
		allWaitUrls.add("http://news.sina.com.cn");
		for (int i = 0; i < 10; i++) {
			MyThread t = new SinaNewsThreadCrawler().new MyThread();
			t.start();
		}
		...
		try {
			...
			if (!allOverUrls.contains(urlStr)) {
				...
				if (depth <= MAX_DEPTH) {
					...
					// 提取有效的数据
					// 判断url是否是需要提取数据的url
					if (urlStr.matches(".*\\d{4}-\\d{2}-\\d{2}.*\\d+\\.shtml")) {
						// 满足要求，再进行数据的提取
						// 根据要求提取标题和描述数据
						// 标题
